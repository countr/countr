...
export default async function chatInputCommandHandler(interaction: ChatInputCommandInteraction<"cached">, document: GuildDocument): Promise<void> {
  ...
  try {
    ...
    if (command.disableInCountingChannel && countingChannel) return void interaction.reply({ content: "❌ This command is disabled in counting channels.", flags: MessageFlags.Ephemeral });

    if (command.requireSelectedCountingChannel && document.channels.size === 0) return void interaction.reply({ content: `💥 No counting channel is set up in this server! Create a new one by using \`/channels new\` or link an existing one with \`/channels link\`. New to Countr? Check out the [documentation](<${docsUrl}>) to get started!`, flags: MessageFlags.Ephemeral });
    ...
    if (command.requireSelectedCountingChannel && !selectedCountingChannel) return void interaction.reply({ content: "💥 You need a counting channel selected to run this command. Type `/select` to select a counting channel and then run this command again.", flags: MessageFlags.Ephemeral });

    return await command.execute(interaction, countingChannel ? 0 : MessageFlags.Ephemeral, document, (selectedCountingChannel ?? [null, null]) as never);
  } catch (err) {
    ...
    if (!interaction.replied && !interaction.deferred) void interaction.reply({ content: "❌ An error occurred while running this command.", flags: MessageFlags.Ephemeral });
  }
}
