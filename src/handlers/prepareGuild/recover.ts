import { PermissionOverwrites, PermissionResolvable, Permissions, TextChannel, ThreadChannel } from "discord.js";
  ...
  if (messages && messages.size) {
    try {
      ...
      while (processing) {
        ...
        if (messages.size) {
          const result = await channel.bulkDelete(messages).catch(() => null);
          if (result) messages = await channel.messages.fetch({ limit: 100, after: lastMessageId });
          else {
            processing = false;
            fail = true;
          }
      ...
      if (channel instanceof TextChannel && oldPermission !== false) {
        await channel.permissionOverwrites.edit(channel.guild.roles.everyone, {
