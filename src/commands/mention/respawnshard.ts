...
export default {
  execute: async (message, args) => {
    ...
    if (!shards.length) return await message.channel.send("❌ No shards were found with your query.");
      ...
      if (shards.includes(thisShard)) return await m.edit(`✅ Restarted shards ${shards.filter(s => s !== thisShard).join(", ")}. (Shard ${thisShard} is restarting after this message is posted)`).then(() => message.client.shard?.send("respawn"));
      else return await m.edit(`✅ Restarted shards ${shards.join(", ")}.`);
    } else return await message.channel.send(`✅ Restarting shard ${shards[0]}.`).then(() => message.client.shard?.broadcastEval(client => client.shard?.send("respawn"), { shard: shards[0] }));
  },
  minArguments: 1
} as MentionCommand;
