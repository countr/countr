  ...
  async execute(interaction, ephemeral, document) {
    ...
    if (threadParent) {
      ...
      if (!currentPermissions.has(requiredPermissions, true)) {
        return void interaction.reply({
          content: `⚠ I am missing permissions in the parent channel <#${threadParent.id}>: ${requiredPermissions.map(bigint => Object.entries(PermissionsBitField.Flags).find(([, permission]) => permission === bigint)?.[0]).join(", ")}`,
          ephemeral: true,
        });
      }
