...
export default {
  ...
  execute: async (interaction, _, { channel: channelId, count, type = Object.keys(numberSystems)[0] }: { channel: string; count: number; type: string; }, document) => {
    ...
    if (document.channels.has(channelId)) return interaction.reply({
    ...
    if (isThread) {
      ...
      if (!parent) return interaction.reply({
      ...
      if (!parent.permissionsFor(interaction.client.user || "", true)?.has(permissions, true)) return interaction.reply({
      ...
      if (!channel.permissionsFor(interaction.client.user || "", true)?.has(permissions, true)) return interaction.reply({
