...
module.exports.run = async (client, message, args, db, permissionLevel, strings, config) => {
    ...
    if (!["each", "only"].includes(mode)) return message.channel.send("❌ " + strings["INVALID_MODE"] + " " + strings["FOR_HELP"].replace("{{HELP}}", "\`" + config.prefix + "help addpin\`"));

    let count = parseInt(args[1]);
    if (!count) return message.channel.send("❌ " + strings["INVALID_COUNT"] + " " + strings["FOR_HELP"].replace("{{HELP}}", "\`" + config.prefix + "help addpin\`"));

    let action = args[2].toLowerCase();
    if (!["keep", "repost"].includes(action)) return message.channel.send("❌ " + strings["INVALID_ACTION"] + " " + strings["FOR_HELP"].replace("{{HELP}}", "\`" + config.prefix + "help addpin\`"));
